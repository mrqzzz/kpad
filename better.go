...
`
	...
	draw := func() {
		...
		for n := ii; n < ii+winHeight; n++ {
			w := len(spl[n])
			if w > winWidth {
				w = winWidth
			}
			st := strings.ReplaceAll(spl[n][:w], ":", tm.Color(":", tm.BLUE))
			if n < ii+winHeight-1 {
				tm.Println(st)
			} else {
				tm.Print(st)
	...
	keyboard.Listen(func(key keys.Key) (stop bool, err error) {
		...
		if key.Code == keys.CtrlShiftDown {
		...
		if key.Code == keys.Down {
			if y >= winHeight && ii < len(spl)-winHeight-1 {
				ii++
				draw()
			}
		...
		if key.Code == keys.Left && x > 0 {
			x--
			tm.MoveCursor(x, y)
			tm.Flush()
		}
		if key.Code == keys.Right && x < winWidth {
			x++
			tm.MoveCursor(x, y)
			tm.Flush()
		}
